...
bool ATMQTT_ConfigureNodes(ATMQTT_Conn_ID_t connID, ATMQTT_Client_ID_t clientID, ATCommon_IP_Addr_t addr, ATCommon_Auth_Username_t username, ATCommon_Auth_Password_t password)
{
    ...
    if (username != NULL)
    {
        // Append username without quotation marks
        if (!ATCommand_AppendArgumentString(pRequestCommand, username, ATCOMMAND_ARGUMENT_DELIM))
        {
            return false;
        }
    ...
    // Remove the optional delimiter if not needed
    if (Adrastea_optionalParamsDelimCount == 0)
    {
        pRequestCommand[strlen(pRequestCommand) - 1] = ATCOMMAND_STRING_TERMINATE;
    }
...
bool ATMQTT_ConfigureTLS(ATMQTT_Conn_ID_t connID, ATCommon_TLS_Auth_Mode_t authMode, ATCommon_TLS_Profile_ID_t profileID)
{
	...
	if (!Adrastea_WaitForConfirm(ADRASTEA_TIMEOUT_MQTT, Adrastea_CNFStatus_Success, NULL))
	{
		return false;
	}
...
bool ATMQTT_ConfigureProtocol(ATMQTT_Conn_ID_t connID, ATMQTT_Keep_Alive_t keepAlive, ATMQTT_Clean_Session_t cleanSession)
{
	...
	if (!Adrastea_WaitForConfirm(ADRASTEA_TIMEOUT_SIM, Adrastea_CNFStatus_Success, NULL))
	{
		return false;
	}
