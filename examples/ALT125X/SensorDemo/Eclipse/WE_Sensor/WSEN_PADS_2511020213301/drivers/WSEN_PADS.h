...
*/
typedef struct {
	uint8_t pads_highPresInt : 1;	/* PHE: enable/disable interrupt on pressure low event (0: disabled; 1:enabled) */
	uint8_t pads_lowPresInt : 1;	/* PLE: enable/disable interrupt on pressure high event (0:disabled; 1:enabled) */
	uint8_t pads_latchedInt : 1;	/* LIR: enable/disable latched Interrupt.(0: normal; 1:enabled) */
	uint8_t pads_diffInt : 1;	/* DIFF_EN  : enable/disable Differential Interrupt generation. (0: disabled; 1:enabled) */
	uint8_t pads_resetAutoZeroFunc : 1;	/* RESET_AZ : Reset Autozero function; reset Reference Pressure Register.(0: normal mode; 1:reset Autozero) */
	uint8_t pads_autoZeroFunc : 1;	/* AUTPZERO : enable/disable Autozero Mode to obtain a reference Pressure stored in Register (0x15,0x16) (0: normal; 1:enabled) */
	uint8_t pads_resetAutoRefp : 1;	/* RESET_ARP: reset Reference Pressure Register. (0: normal mode; 1:enabled) */
	uint8_t pads_autoRefp : 1;	/* AUTOREP  : enable AUTOREFP function. (0: disabled; 1:enabled) */
...
*/
typedef struct {
	uint8_t pads_presThresholdLsb : 8;	/* THR[7:0] This register contains the low part of threshold value for pressure interrupt */
...
*/
typedef struct {
	uint8_t pads_presThresholdMsb : 7;	/* THR[14:8] This register contains the high part of threshold value for pressure interrupt */
	uint8_t pads_notUsed01 : 1;	/* This bit must be set to 0 for proper operation of the device */
...
*/
typedef struct {
	uint8_t pads_disableI2C : 1;   /* i2cDisable : enable/disable I2C digital Interface.(0: disabled; 1:enabled) */
	uint8_t pads_notUsed01 : 1;   /* This bit must be set to 0 for proper operation of the device*/
	uint8_t pads_disPullDownOnIntPin : 1;   /* PD_DIS_INT : enable/disable pull down on the INT pin.(0:enabled; 1:disabled) */
	uint8_t pads_pullUpOnSAOpin : 1;   /* SAO_PU_EN enable/disable  pull-up on the SAO pin.(0: disabled; 1:enabled) */
	uint8_t pads_pullUpOnSDApin : 1;   /* SDA_PU_EN enable/disable  pull-up on the SDA pin.(0: disabled; 1:enabled) */
	uint8_t pads_notUsed02 : 2;   /* This bit must be set to 0 for proper operation of the device */
	uint8_t pads_notUsed03 : 1;   /* This bit must be set to 0 for proper operation of the device */
...
typedef struct {
	uint8_t pads_notUsed01 : 1;  /* This bit must be set to 0 for proper operation of the device */
	uint8_t pads_blockDataUpdate : 1;  /* BDU : block data update. 0 - continuous update; 1 - output registers not update till both MSB and LSB are read */
	uint8_t pads_lowPassFilterConfig : 1;  /* LPFP_CFG : Configure Low Pass filter on Pressure data */
	uint8_t pads_enLowPassFilter : 1;  /* EN_LPFP : enable and Low Pass filter on Pressure data */
	uint8_t pads_outputDataRate : 3;  /* ODR[2:0]: output data rate.Default '000' */
	uint8_t pads_notUsed02 : 1;  /* This bit must be set to 0 for proper operation of the device */
} PADS_ctrl_1_t;
...
*/
typedef struct {
	uint8_t pads_oneShotbit : 1; /* ONE_SHOT : 0  Nornal Operation; 1: start Measurement */
	uint8_t pads_lowNoiseMode : 1; /* LOW_NOISE_EN : Enables low noise mode (used only if ODR is lower than 100 Hz). Default value: 0 (0: low-power mode; 1: low-noise mode) */
	uint8_t pads_softwareReset : 1; /* swreset :Software reset. 0: normal mode; 1: SW reset;  Self-clearing upon completion */
	uint8_t pads_notUsed01 : 1; /* This bit must be set to 0 for proper operation of the device*/
	uint8_t pads_autoAddIncr : 1; /* IF_ADD_INC : Register address automatically incremented during a multiple byte access with I2C interface. Default value 1.( 0: disable; 1: enable) */
	uint8_t pads_openDrainOnINTpin : 1; /* PP_OD: Push-pull/open-drain selection on interrupt pad. Default value: 0 (0: push-pull; 1: open-drain) */
	uint8_t pads_intActiveLevel : 1; /* INT_H_L : Interrupt active High , active low . Default value : 0 (0:active High; 1:active low ) */
	uint8_t pads_boot : 1; /* BOOT :Reboot memory content. 0: normal mode; 1: reboot memory content. Self-clearing upon completion */
} PADS_ctrl_2_t;
...
*/
typedef struct {
	uint8_t pads_intEventCtrl : 2; /* INT_S:data signal on INT pad control bits :Default value: 00 */
	uint8_t pads_dataReadyInt : 1; /* DRDY : Data-ready signal on INT pin.Default value: 0. (0: Disable; 1 : Enable) */
	uint8_t pads_fifoOverrunInt : 1; /* INT_F_OVR  :enable  FIFO overrun Interrupt. Default value: 0.(0: Disable; 1 : Enable) */
	uint8_t pads_fifoThresholdInt : 1; /* INT_F_WTM  : enable  FIFO threshold (watermark). Default value: 0 (0: Disable; 1 : Enable) */
	uint8_t pads_fifoFullInt : 1; /* INT_F_FULL : enable  FIFO FUll .Default value: 0; (0: Disable; 1 : Enable) */
	uint8_t pads_notUsed01 : 2; /* This bit must be set to 0 for proper operation of the device */
} PADS_ctrl_3_t;
...
*/
typedef struct {
	uint8_t pads_fifoMode : 3; /* [ TRIG_MODES;FMODE[1:0] ]  : select fifo_mode */
	uint8_t pads_stopOnThreshold : 1; /* STOP_ON_WTM : Enable  FIFO watermark level use. Default value: 0(0: disabled; 1: enabled) */
	uint8_t pads_notUsed01 : 4; /* These 4 bits must be set to 0 for proper operation of the device */
} PADS_fifo_ctrl_t;
...
*/
typedef struct {
	uint8_t pads_fifoThreshold : 7; /* WTM[6:0] : FIFO threshold level setting. Default value: 0x00 */
	uint8_t pads_notUsed01 : 1; /* This bit must be set to 0 for proper operation of the device */
} PADS_fifo_wtm_t;
...
*/
typedef struct {
	uint8_t pads_diffPresHighEvent : 1; /* PH : Differential pressure High.(0: no interrupt; 1: high differential pressure event has occurred) */
	uint8_t pads_diffPresLowEvent : 1; /* PL : Differential pressure Low.(0: no interrupt;1: low differential pressure event has occurred) */
	uint8_t pads_intStatus : 1; /* IA  : Interrupt active.(0: no interrupt;1: one or more interrupt events have been generated) */
	uint8_t pads_notUsed01 : 4; /* These 4 bits must be set to 0 for proper operation of the device*/
	uint8_t pads_bootOn : 1; /* BOOT_ON : Indication of Boot phase. (0: Boot phase has ended; 1: Boot phase is running) */
} PADS_intSource_t;
...
*/
typedef struct {
	uint8_t pads_notUsed01 : 5; /* These 5 bits must be set to 0 for proper operation of the device */
	uint8_t pads_fifoFull : 1; /* FIFO_FULL_IA :FIFO full status.(0: FIFO not full; 1: FIFO is full, no samples overwritten) */
	uint8_t pads_fifoOverrun : 1; /* FIFO_OVR_IA :FIFO overrun status. (0: FIFO is not overrun; 1: FIFO is full and at least one sample has been overwritten) */
	uint8_t pads_fifoWtm : 1; /* FIFO_WTM_IA : threshold (watermark) status.(0: FIFO level below threshold; 1: FIFO  level equal or higher than treshold) */
} PADS_fifo_status2_t;
...
*/
typedef struct {
	uint8_t pads_presDataAvailable : 1; /* P_DA : Pressure data available.(0: pressure data not yet available; 1: a new pressure data is available) */
	uint8_t pads_tempDataAvailable : 1; /* T_DA :Temperature data available. (0: temperature not yet available; 1: a new temperature data is available) */
	uint8_t pads_notUsed01 : 2; /* These 2 bits must be set to 0 for proper operation of the device */
	uint8_t pads_presDataOverrun : 1; /* P_OR : Pressure data overrun. (0: no overrun; 1: pressure data overwritten) */
	uint8_t pads_tempDataOverrun : 1; /* T_OR : Temperature data overrun. (0: no overrun; 1: Temperature data overwritten) */
	uint8_t pads_notUsed02 : 2; /* This 2 bits must be set to 0 for proper operation of the device */
} PADS_status_t;
...
extern "C"
{
	...
	int8_t PADS_getTemperature(float *tempdegC);  // Temperature Value in Â°C

	/*Fifo Data Out */
	int8_t PADS_getFifoRAWTemperature(int16_t *rawTemp);
	int8_t PADS_getFifoRAWPressure(int32_t *rawPres);
	int8_t PADS_getFifoTemperature(float *tempdegC);  // Temperature Value in Â°C
