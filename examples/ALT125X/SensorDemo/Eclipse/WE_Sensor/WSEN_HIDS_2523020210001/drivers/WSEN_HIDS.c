...
uint8_t HIDS_getTempAvg(HIDS_average_conf_t *avgT)
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_Average_REG, 1, (uint8_t *)&average_reg))
	return WE_FAIL;
...
int8_t HIDS_setOdr(HIDS_output_data_rate_t odr)
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_CTRL_REG_1, 1, (uint8_t *)&ctrl_reg_1))
	return WE_FAIL;

	ctrl_reg_1.odr = odr;

	if (I2C_WriteReg((uint8_t)HIDS_CTRL_REG_1, 1, (uint8_t *)&ctrl_reg_1))
	return WE_FAIL;
...
int8_t HIDS_enOneShot(HIDS_state_t oneShot)
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_CTRL_REG_2, 1, (uint8_t *)&ctrl_reg_2))
	return WE_FAIL;

	ctrl_reg_2.oneShotBit = oneShot;

	if (I2C_WriteReg((uint8_t)HIDS_CTRL_REG_2, 1, (uint8_t *)&ctrl_reg_2))
	return WE_FAIL;
...
int8_t HIDS_enInt(HIDS_state_t Drdy)
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_CTRL_REG_3, 1, (uint8_t *)&ctrl_reg_3))
	return WE_FAIL;

	ctrl_reg_3.enDataReady = Drdy;

	if (I2C_WriteReg((uint8_t)HIDS_CTRL_REG_3, 1, (uint8_t *)&ctrl_reg_3))
	return WE_FAIL;
...
int8_t HIDS_getDeviceID(uint8_t *devID)
{
	...
	if (WE_FAIL == I2C_ReadReg((uint8_t)HIDS_DEVICE_ID_REG, 1, devID))
	return WE_FAIL;
...
int8_t HIDS_getHumStatus(HIDS_state_t *humidity_state)
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_STATUS_REG, 1, (uint8_t *)&status_reg))
	return WE_FAIL;
...
int8_t HIDS_getRAWValues(int16_t *rawHumidity, int16_t *rawTemp)
{
	...
	uint8_t raw[4] = {0};
	if (I2C_ReadReg((uint8_t)HIDS_H_OUT_L_REG, 4, (uint8_t *)&raw))
	return WE_FAIL;
...
uint16_t HIDS_get_H_T_out()
{
	uint8_t buffer;
	uint16_t H_T_out = 0;
	if (I2C_ReadReg(HIDS_H_OUT_H_REG, 1, &buffer))
	return WE_FAIL;
	H_T_out = (uint16_t)buffer << 8;

	if (I2C_ReadReg(HIDS_H_OUT_L_REG, 1, &buffer))
	return WE_FAIL;
	H_T_out |= buffer;
...
uint16_t HIDS_get_H0_T0_out()
{
	...
	if (I2C_ReadReg(HIDS_H0_T0_OUT_H, 1, &buffer))
	return WE_FAIL;
	H0_T0_out = (((uint16_t)buffer) << 8);

	if (I2C_ReadReg(HIDS_H0_T0_OUT_L, 1, &buffer))
	return WE_FAIL;
	H0_T0_out |= buffer;
...
uint16_t HIDS_get_H1_T0_out()
{
	...
	if (I2C_ReadReg(HIDS_H1_T0_OUT_H, 1, &buffer))
	return WE_FAIL;
	H1_T0_out = (((uint16_t)buffer) << 8);

	if (I2C_ReadReg(HIDS_H1_T0_OUT_L, 1, &buffer))
	return WE_FAIL;
	H1_T0_out |= buffer;
...
uint16_t HIDS_get_H0_rh()
{
	...
	if (I2C_ReadReg(HIDS_H0_RH_X2, 1, &H0_rh))
	return WE_FAIL;
...
uint16_t HIDS_get_H1_rh()
{
	...
	if (I2C_ReadReg((uint8_t)HIDS_H1_RH_X2, 1, &H1_rh))
	return WE_FAIL;
...
uint16_t HIDS_get_T_OUT()
{
	uint8_t buffer;
	uint16_t T_OUT = 0;
	if (I2C_ReadReg(HIDS_T_OUT_H_REG, 1, &buffer))
	return WE_FAIL;
	T_OUT = (uint16_t)buffer << 8;

	if (I2C_ReadReg(HIDS_T_OUT_L_REG, 1, &buffer))
	return WE_FAIL;
	T_OUT |= buffer;
...
uint16_t HIDS_get_T1_OUT()
{
	uint8_t buffer;
	uint16_t T1_OUT = 0;
	if (I2C_ReadReg(HIDS_T1_OUT_H, 1, &buffer))
	return WE_FAIL;
	T1_OUT = ((uint16_t)buffer) << 8;

	if (I2C_ReadReg(HIDS_T1_OUT_L, 1, &buffer))
	return WE_FAIL;
	T1_OUT |= buffer;
...
uint16_t HIDS_get_T0_OUT()
{
	uint8_t buffer;
	uint16_t T0_OUT = 0;
	if (I2C_ReadReg(HIDS_T0_OUT_H, 1, &buffer))
	return WE_FAIL;
	T0_OUT = ((uint16_t)buffer) << 8;

	if (I2C_ReadReg(HIDS_T0_OUT_L, 1, &buffer))
	return WE_FAIL;
	T0_OUT |= buffer;
...
void SW_RESET()
{
HIDS_enRebootMemory(HIDS_enable);
vTaskDelay(1000);
HIDS_enRebootMemory(HIDS_disable);
vTaskDelay(1000);
}
...
int8_t HIDS_getHumidity(float *humidity)
{
	...
	*humidity = hum + h_temp; // provide signed % measurement unit
	printf("Relative temp is :%f ", humidity);
	return WE_SUCCESS;
}
/**
...
int8_t HIDS_getTemperature(float *tempDegC)
{
	...
	if (I2C_ReadReg(HIDS_T0_T1_DEGC_H2, 1, &tmp))
	return WE_FAIL;

	if (I2C_ReadReg(HIDS_T0_DEGC_X8, 1, &buffer))
	return WE_FAIL;
	...
	if (I2C_ReadReg(HIDS_T1_DEGC_X8, 1, &buffer))
	return WE_FAIL;
