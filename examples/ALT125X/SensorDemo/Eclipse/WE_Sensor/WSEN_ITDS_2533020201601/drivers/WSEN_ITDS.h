...
/**         ITDS 2521020222501 DEVICE_ID         */
//#define ITDS_DEVICE_ID_VALUE				(uint8_t)0xB3 		/* Device ID of ITDS 2521020222501 Sensor */
...
typedef struct {
	...
	uint8_t boot : 1;             /* BOOT : This bit is set to â€™1â€™ during boot sequence.default value:0 (0: enabled ; 1:disabled) .Self-clearing upon completion */
}ITDS_ctrl_2_t;
...
typedef struct
{
	uint8_t fifoFillLevel : 6;       /* Diff[5:0] :Defines the number of unread samples stored in FIFO. (â€™000000â€™ = FIFO empty, â€™100000â€™ = FIFO full, 32 unread samples) */
...
typedef struct
{
	uint8_t XAxisTapThreshold : 5;    /* TAP_THSX_[4:0] :Threshold for TAP recognition at FS = Â±2g on X direction */
...
typedef struct
{
	uint8_t  YAxisTapThreshold : 5;  /* TAP_THSY_[4:0] :Threshold for TAP recognition at FS = Â±2g on Y direction */
...
typedef struct
{
	uint8_t ZAxisTapThreshold : 5;  /* TAP_THSZ_[4:0] Threshold for tap recognition at FS: Â±2g on Z direction. */
...
typedef struct
{
	...
	uint8_t  userOffset : 1;          /* USR_OFF_W :Defines the selection of weight of the user offset words specified byX_OFS_USR[7:0], Y_OFS_USR[7:0] and Z_OFS_USR[7:0] bits (0:977 Âµg /LSB, 1: 15.6 mg /LSB) */
	uint8_t  applyWakeUpOffsetXL : 1; /* USR_OFF_ON_WU :Enable application of user offset value on XL data for wakeup function only */
	uint8_t  applyOffsetXL : 1;       /* USR_OFF_ON_OUT :Enable application of user offset value on XL output data registers.FDS :bit in CTRL_6 (0x25) must be set to â€™0â€™-logic (low-pass path selected) */
