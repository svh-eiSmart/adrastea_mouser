...
bool ATCommand_IntToString(char *outString, uint32_t number, uint16_t intFlags) {
	if ((0 == (intFlags & ATCOMMAND_INTFLAGS_SIGN ))
			|| (0 == (intFlags & ATCOMMAND_INTFLAGS_NOTATION ))) {
		return false;
	}

	if (ATCOMMAND_INTFLAGS_NOTATION_HEX
			== (intFlags & ATCOMMAND_INTFLAGS_NOTATION )) {
		/* HEX */
		sprintf(outString, "0x%lx", number);
	} else {
		/* DEC */

		if (ATCOMMAND_INTFLAGS_UNSIGNED
				== (intFlags & ATCOMMAND_INTFLAGS_SIGN )) {
			/* UNSIGNED */
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_StringToInt(void *number, const char *inString,
		uint16_t intFlags) {
	if ((NULL == inString) || (NULL == number)) {
		return false;
	}

	bool hex = (0 == strncmp(inString, "0x", 2))
			|| ((intFlags & ATCOMMAND_INTFLAGS_NOTATION_HEX ) != 0);
	...
	if (signedDataType) {
		...
		unsigned long long longNr = strtoull(inString, &endptr,
				(hex == true) ? 16 : 10);
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentBytes(char *pOutString, const char *pInArgument,
		uint16_t numBytes, char delimiter) {
	if (NULL == pOutString) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentString(char *pOutString, const char *pInArgument,
		char delimiter) {
	if (NULL == pOutString) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentStringQuotationMarks(char *pOutString,
		const char *pInArgument, char delimiter) {
	if (NULL == pOutString) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentBitmask(char *pOutString, const char *stringList[],
		uint8_t numStrings, uint32_t bitmask, char delimiter,
	...
	for (int i = 0; i < numStrings; i++) {
		if (0 != (bitmask & (1 << i))) {
			...
			if (!ATCommand_AppendArgumentString(pOutString, stringList[i],
					ATCOMMAND_BITMASK_DELIM)) {
				return false;
			}
			empty = false;
		}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentBoolean(char *pOutString, bool inBool,
		char delimiter) {
	return ATCommand_AppendArgumentString(pOutString,
			ATCommand_BooleanValueStrings[(inBool == true) ? 1 : 0], delimiter);
}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentString(char **pInArguments, char *pOutArgument,
		char delimiter, uint16_t maxLength) {
...
bool ATCommand_GetNextArgumentStringWithoutQuotationMarks(char **pInArguments,
		char *pOutArgument, char delimiter, uint16_t maxLength) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentInt(char *pOutString, uint32_t pInValue,
		uint16_t intFlags, char delimiter) {
	if (NULL == pOutString) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_AppendArgumentIntQuotationMarks(char *pOutString,
		uint32_t pInValue, uint16_t intFlags, char delimiter) {
	if (NULL == pOutString) {
		return false;
	}
	...
	if (ATCommand_IntToString(tempString, pInValue, intFlags)) {
		return ATCommand_AppendArgumentStringQuotationMarks(pOutString,
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentEnum(char **pInArguments, uint8_t *pOutArgument,
		const char *stringList[], uint8_t numStrings, uint16_t maxStringLength,
		char delimiter) {
	char tempString[maxStringLength];

	if (!ATCommand_GetNextArgumentString(pInArguments, tempString, delimiter,
			sizeof(tempString))) {
		return false;
	}

	bool ok;
	*pOutArgument = ATCommand_FindString(stringList, numStrings, tempString, 0,
			&ok);
	return ok;
}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentByteArray(char **pInArguments, uint16_t length,
		uint8_t *pOutArgument, uint16_t maxLength) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
	if (length > maxLength) {
...
bool ATCommand_GetNextArgumentBitmask(char **pInArguments,
		const char *stringList[], uint8_t numStrings, uint16_t maxStringLength,
		uint32_t *bitmask, char delimiter) {
	...
	*bitmask = 0;
	...
	while (**pInArguments != '\0'
			&& ((ret = ATCommand_GetNextArgumentString(pInArguments, tempString,
					ATCOMMAND_BITMASK_DELIM, sizeof(tempString)))
					|| (ret = ATCommand_GetNextArgumentString(pInArguments,
							tempString, delimiter, sizeof(tempString))))) {
		bool ok;
		uint8_t flag = ATCommand_FindString(stringList, numStrings, tempString,
				0, &ok);
		if (ok) {
			*bitmask |= (1 << flag);
		}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentBoolean(char **pInArguments, bool *outBool,
		char delimiter) {
	uint8_t enumValue;
	bool ok = ATCommand_GetNextArgumentEnum(pInArguments, &enumValue,
			ATCommand_BooleanValueStrings,
			ATCommand_BooleanValue_NumberOfValues, 6, delimiter);
	if (ok) {
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentInt(char **pInArguments, void *pOutArgument,
		uint16_t intFlags, char delimiter) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
	...
	ATCommand_GetNextArgumentString(pInArguments, tempString, delimiter,
			sizeof(tempString));
...
bool ATCommand_GetNextArgumentIntWithoutQuotationMarks(char **pInArguments,
		void *pOutArgument, uint16_t intFlags, char delimiter) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
	...
	ATCommand_GetNextArgumentStringWithoutQuotationMarks(pInArguments,
			tempString, delimiter, sizeof(tempString));
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetCmdName(char **pInAtCmd, char *pCmdName, char *delimiters,
 ...
 */
uint8_t ATCommand_FindString(const char *stringList[], uint8_t numStrings,
		const char *str, uint8_t defaultValue,
		bool *ok) {
...
bool ATCommand_AppendArgumentBitsQuotationMarks(char *pOutString,
	...
	return ATCommand_AppendArgumentStringQuotationMarks(pOutString, tempString,
			delimiter);
...
bool ATCommand_GetNextArgumentEnumWithoutQuotationMarks(char **pInArguments,
		uint8_t *pOutArgument, const char *stringList[], uint8_t numStrings,
		uint16_t maxStringLength, char delimiter) {
	char tempString[maxStringLength];
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentDouble(char **pInArguments, void *pOutArgument,
		char delimiter) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool ATCommand_GetNextArgumentFloat(char **pInArguments, void *pOutArgument,
		char delimiter) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
...
bool ATCommand_GetNextArgumentDoubleWithoutQuotationMarks(char **pInArguments,
		void *pOutArgument, char delimiter) {
	if ((NULL == pInArguments) || (NULL == pOutArgument)) {
		return false;
	}
