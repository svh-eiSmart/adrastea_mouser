...
{
	if (WE_FAIL == I2C_ReadReg((uint8_t)TIDS_DEVICE_ID_REG, 1, devID))
	return WE_FAIL;
...
int8_t TIDS_setSwReset(TIDS_state_t mode)
{
	...
	if (WE_FAIL == I2C_WriteReg((uint8_t)TIDS_SOFT_RESET_REG, 1, (uint8_t *)&swRstReg))
	return WE_FAIL;
...
int8_t TIDS_setContinuousMode(TIDS_state_t mode)
{
	...
	if (WE_FAIL == I2C_WriteReg((uint8_t)TIDS_CTRL_REG, 1, (uint8_t *)&ctrl_reg))
	return WE_FAIL;
...
int8_t TIDS_getTemperature(float *tempdegC)
{
	...
	*tempdegC = (float)rawTemp;
	// printf("tempdegC after conversion: %f\n", *tempdegC);

	*tempdegC = *tempdegC / 100;
	// printf("tempdegC after scaling: %f\n", *tempdegC);
...
{
	if (WE_FAIL == I2C_WriteReg((uint8_t)TIDS_LIMIT_T_H_REG, 1, &hLimit))
	return WE_FAIL;
...
{
	if (WE_FAIL == I2C_WriteReg((uint8_t)TIDS_LIMIT_T_L_REG, 1, &lLimit))
	return WE_FAIL;
