...
static serial_handle sio_init(lpuart_rtos_config_t *lpuart_config)
{
    serial_handle sh;
    setPPPEscapeString_sio(DEF_ESCAPE_STR,3,DEF_TIME_BEFORE_ESCAPE,DEF_TIME_AFTER_ESCAPE);

    sh = serialDriverContainerOpen(lpuart_config);
    serialDriverContainerIoctl(sh, IOCTL_SERIAL_RX_CB_FUNC,
               			         (void *)sendToPpp, NULL);
    return sh;
}
...
{
    lpuart_rtos_config_t *lpuart_config = (lpuart_rtos_config_t *) sio_arg;
    serial_handle sh = NULL;

    //print_boot_msg("sio_open: for muxID %d virtualPortID=%d\n", mux_id, virtual_port_id);

    if(( sh = sio_init(lpuart_config)) == NULL)
    {
...
u32_t sio_write(sio_fd_t fd, u8_t *data, u32_t len)
{
    ...
    ret = serialDriverContainerWrite((serial_handle)fd, (const uint8_t *)data, len);
    if (ret == SERIAL_CONTAINER_SUCCESS)
        return len;
    else
        return 0;
...
*/
    ...
    serialDriverContainerWrite((serial_handle)fd, (const uint8_t *)&c, 1);
}
#if 0
